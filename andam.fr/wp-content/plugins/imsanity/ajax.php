	...
	if ($images)
	{
		...
		if (($oldW > $maxW && $maxW > 0) || ($oldH > $maxH && $maxH > 0))
		{
			...
			if (!is_wp_error($resizeResult))
			{
				...
				$results = array('success'=>true,'id'=> $id, 'message' => 'OK: ' . $oldPath);
			}
			else
			{
				$results = array('success'=>false,'id'=> $id, 'message' => 'ERROR: ' . $oldPath . ' (' . htmlentities($resizeResult->get_error_message()) . ')');
			}
		}
		else
		{
			$results = array('success'=>true,'id'=> $id, 'message' => 'SKIPPED: ' . $oldPath . ' (Resize not required)');
		}

	}
	else
	{
		$results = array('success'=>false,'id'=> $id, 'message' => 'ERROR: (Attachment with ID of ' . htmlentities($id) . ' not found)');
	}
