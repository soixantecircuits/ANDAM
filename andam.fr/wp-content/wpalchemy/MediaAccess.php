 ...
 class WPAlchemy_MediaAccess
{
	...
	public function getButtonLink($tab = null)
	{
		...
		return 'media-upload.php?post_id=' . $post_ID . '&mic_action=select_file&tab=' . $tab . '&TB_iframe=1';
	}
...
/////////////////////////////////////////////////////////////////////////////////
// Added by Dameian on 4/11/2011 to allow for extraction of the attachment's ID 
// and population of a preview area.
			
// Isolate the attachment's ID
var mediaID = html.match(/wp-image-(.*)"/i);
mediaID = (mediaID && mediaID[1]) ? mediaID[1] : '' ;
send_myid(mediaID, wpalchemy_mediafield_selector);		
// Pass the attachment ID to a hidden field with ID #media_id
$(wpalchemy_mediafield_selector).closest('.wpa_group').find('#media_id').val(mediaID);
$(wpalchemy_mediafield_selector).val(mediaID);
			
// Send a preview thumb to a preview area
//$(wpalchemy_mediafield_selector).closest('.wpa_group').find('.preview').html(html);
			
// Maybe make our media ID global and available to things like AJAX?
// WPAlchemyMediaID = mediaID;
